...
module.exports.createFileUpload = function (options, type) {

  // If no adapter passed in, assume options is just an upload dir
  var adapters = { 'file':1, 's3':1 }
    , uploadDelegate
  if (type in adapters)
    uploadDelegate =  require('./modules/' + type)(options)
  else
    uploadDelegate = options.adapter || require('./modules/file')(options)
